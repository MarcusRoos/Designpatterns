//------------------------------------------------------------------------------
// HanoiEngine.cpp DT063G Design Patterns With C++
// Implementation of class HanoiEngine
//------------------------------------------------------------------------------

#include <iostream>
#include <iomanip>
#include "../include/HanoiEngine.h"

using std::setw;
using std::right;
using std::endl;

HanoiEngine::HanoiEngine(int aDiscs) : iDiscs(aDiscs), iMoves(0) {
    for (size_t i = iDiscs; i > 0; --i)
...
void HanoiEngine::show(ostream &aStrm) {
    aStrm << '\n';
    for (size_t row = 0; row < iDiscs; ++row) {
        for (size_t tower = 0; tower < 3; ++tower) {
            if ((iDiscs-row) > iTowers[tower].size())
                aStrm << '\t' << setw(2) << right << '.';
            else {
                size_t index = row-iDiscs+iTowers[tower].size();
...
bool HanoiEngine::move(int aFrom, int aTo) {
    ...
    if (iTowers[aFrom-1].size() &&
       (!iTowers[aTo-1].size() ||
       iTowers[aFrom-1].front() < iTowers[aTo-1].front())) {
        // Yes!
